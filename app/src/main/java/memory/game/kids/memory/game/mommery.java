...
public class mommery {
    ...
    private Tile mT1;
    private Tile mT2;
    TileList mList = new TileList();
    private int[] mTiles;
    private Memory_two.OnMemoryListener mListener;
    private static int[] mSounds;
    private int mTileVerso;

    public mommery(int[] tiles, int[] sounds, int tile_verso, Memory_two.OnMemoryListener listener)
    {
        mTiles = tiles;
        mSounds = sounds;
        mListener = listener;
...
//                    playSound( SOUND_FAILED );
    ...
    private void addRandomly(int nResId)
    {
        ...
        for (int y = 0 ; y <mList.size() ; y ++){
            mList.add(y, new Tile(nResId));
        }

        dPos = Math.random() * mList.size();
        nPos = (int) dPos;
        for (int h = 0 ; h <mList.size() ; h++){
            mList.add(h, new Tile(nResId));
        }
